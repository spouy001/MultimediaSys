...
//			Matrix classXY=XY.getMatrix(colD-classNum,colD-1,0,1);
			...
			for(int i=0;i<rowDim;i++){
				for(int j=0;j<colDim;j++){
					...
					//final_MCA_weight[i][j]=1.0*MCA_weight.get(i,j)/(Attr_columnTotal.get(i,0)*class_columnTotal.get(j,0)-MCA_weight.get(i,j));
				}				
			}
			...
			final_tanimoto_weight = new double[rowDim][colDim];
			for(int i=0;i<rowDim;i++){
				for(int j=0;j<colDim;j++){
					// calculate weight based on Tanimoto coefficients
